    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (fileData) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self _finishWithURLSchemeTask:urlSchemeTask fileData:fileData URL:URL];
            });
        } else {
            NSLog(@"[webFast] startURLSchemeTask_remoteResource: %@", URL);
            
            [self _dataTaskWithRequest:URLRequest urlSchemeTask:urlSchemeTask];
        }
    });
}
...
- (void)_dataTaskWithRequest:(NSMutableURLRequest *)URLRequest urlSchemeTask:(id <WKURLSchemeTask>)urlSchemeTask API_AVAILABLE(ios(11.0)) {
...
- (void)_finishWithURLSchemeTask:(id <WKURLSchemeTask>)urlSchemeTask fileData:(NSData *)fileData URL:(NSURL *)URL API_AVAILABLE(ios(11.0)) {
    if ([self.availableTasks containsObject:urlSchemeTask]) {
...
- (void)_failWithURLSchemeTask:(id <WKURLSchemeTask>)urlSchemeTask fileData:(NSData *)fileData error:(NSError *)error URL:(NSURL *)URL API_AVAILABLE(ios(11.0)) {
    if ([self.availableTasks containsObject:urlSchemeTask]) {
        NSMutableDictionary *headerFields = [NSMutableDictionary dictionaryWithDictionary:@{@"Access-Control-Allow-Origin": @"*"}];
        [headerFields wfSetObject:[MKWebFastURLSchemeContentTypes() wfStringForKey:URL.pathExtension] forKey:@"Content-Type"];
